...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
	...
	__HAL_UART_ENABLE_IT(&huart2, UART_IT_IDLE); //使能串UART1 IDLE(空闲)中断
	HAL_UART_Receive_DMA(&huart2, RX2_Buf, DMA_BUF_SIZE); //设置DMA传输，将uart1的数据搬运到RX2_Buf中
	...
	OLED_Init();
	OLED_CLS(); // 清除屏幕显示
	OLED_ShowStr(16,0, (unsigned char*)"FreeRTOS All", 2);	
	AHT20_Init();
	INA226_Init();
