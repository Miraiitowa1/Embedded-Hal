...
void RTC_Test(void)
{
	struct tm  *tp;	
	uint8_t str[64];
	//OLED_ShowStr(16,0, (unsigned char*)"RTC Test", 2);	
	time_t t = RTC_ReadTimeCounter(&hrtc); // t 就是秒数 
	tp = localtime(&t) ; // 把秒数转换成时间结构体
	printf("%u::%d-%d-%d %d:%d:%d\n",t,tp->tm_year+1900,tp->tm_mon+1,tp->tm_mday,
							 tp->tm_hour,tp->tm_min,tp->tm_sec);
	
	sprintf((char *)str,"Date:%04d-%02d-%02d",tp->tm_year+1900,tp->tm_mon+1,tp->tm_mday);
	OLED_ShowStr(0,4, (unsigned char*)str, 2);	
	sprintf((char *)str,"Time:%02d:%02d:%02d",tp->tm_hour,tp->tm_min,tp->tm_sec);
